...
function createRadarPoint(point) {
  ...
  pointElement.style.left = `calc(50% + ${
    point.radius * 0.5 * Math.cos((point.angle * Math.PI) / 180)
  }%)`;
  pointElement.style.top = `calc(50% + ${
    point.radius * 0.5 * Math.sin((point.angle * Math.PI) / 180)
  }%)`;
